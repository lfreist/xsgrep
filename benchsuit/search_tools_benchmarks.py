...
def benchmark_literal_byte_offset(pattern: str, iterations: int, drop_cache: bool) -> GNUTimeBenchmark:
    ...
    commands = [
        ...
        GNUTimeCommand("xs grep meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path, "-b"], pre_cmd),
        GNUTimeCommand("xs grep -j 1", ["xs", pattern, DATA_FILE_PATH, "-j", "1", "-b"], pre_cmd),
        GNUTimeCommand("xs grep -j 1 meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path, "-j", "1", "-b"], pre_cmd),
...
def benchmark_literal_line_number(pattern: str, iterations: int, drop_cache: bool) -> GNUTimeBenchmark:
    ...
    commands = [
        ...
        GNUTimeCommand("xs grep meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path, "-n"], pre_cmd),
        GNUTimeCommand("xs grep -j 1", ["xs", pattern, DATA_FILE_PATH, "-j", "1", "-n"], pre_cmd),
        GNUTimeCommand("xs grep -j 1 meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path, "-j", "1", "-n"], pre_cmd),
...
def benchmark_literal(pattern: str, iterations: int, drop_cache: bool) -> GNUTimeBenchmark:
    ...
    commands = [
        ...
        GNUTimeCommand("xs grep meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path], pre_cmd),
        GNUTimeCommand("xs grep -j 1", ["xs", pattern, DATA_FILE_PATH, "-j", "1"], pre_cmd),
        GNUTimeCommand("xs grep -j 1 meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path, "-j", "1"], pre_cmd),
...
def benchmark_literal_case_insensitive(pattern: str, iterations: int, drop_cache: bool) -> GNUTimeBenchmark:
    ...
    commands = [
        ...
        GNUTimeCommand("xs grep meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path, "-i"], pre_cmd),
        GNUTimeCommand("xs grep -j 1", ["xs", pattern, DATA_FILE_PATH, "-j", "1", "-i"], pre_cmd),
        GNUTimeCommand("xs grep -j 1 meta", ["xs", pattern, DATA_FILE_PATH, "-m", meta_file_path, "-j", "1", "-i"], pre_cmd),
...
def benchmark_zstd_input(pattern: str, iterations: int, drop_cache: bool) -> GNUTimeBenchmark:
    ...
    commands = [
        ...
        GNUTimeCommand("xs grep meta", ["xs", pattern, xs_zstd_file, "-m", xs_zstd_meta], xs_pre_cmd),
        GNUTimeCommand("zstdcat | xs grep -j 1", f"zstdcat {zstd_file} | xs {pattern} -j 1", pre_cmd),
        GNUTimeCommand("xs grep -j 1 meta", ["xs", pattern, xs_zstd_file, "-m", xs_zstd_meta, "-j", "1"], xs_pre_cmd),
...
def benchmark_lz4_input(pattern: str, iterations: int, drop_cache: bool) -> GNUTimeBenchmark:
    ...
    commands = [
        ...
        GNUTimeCommand("xs grep meta", ["xs", pattern, xs_lz4_file, "-m", xs_lz4_meta], xs_pre_cmd),
        GNUTimeCommand("zstdcat | xs grep -j 1", f"lz4cat {lz4_file} | xs {pattern} -j 1", pre_cmd),
        GNUTimeCommand("xs grep -j 1 meta", ["xs", pattern, xs_lz4_file, "-m", xs_lz4_meta, "-j", "1"], xs_pre_cmd),
    ...
    for name, bm_func in benchmarks.items():
        ...
        if res:
            if args.output:
                ...
                result_info_data[file_name + ".json"]["plot"] = output_file + ".pdf"
